...
nFilesMeta = 2;max(nFiles,1); % number of files to read to determine metadata from;
flag = 0;
for k = 1 : length(d)
...
%%
%meta(1).stacksize = 800;
initial_stackSize = 200;
t_off = 0;
...
counter   = 1;
ncol      = 2; % meta(1).nchan; %Set number of colors
TimeCount = 0;

%if saving to folder which is different from the current one:
%savepath = 'H:\npmitchell\202208101028_180s_1p4um_2mW2mW_48YG4knlsGFPCAAXmCh';
savepath = pwd ;
datafolder = fullfile(savepath, 'data');
...
for p = 1:length(pos_todo)
    angle = pos_todo(p)*angle_increment + angle_offset ;
    ...
    for time = TInit:TEnd
        ...
        for k = 1 : filesize     
            ...
            if ismember(TimeCount, timestamps_to_save)
                % read image, 
                temp =  imread(name,k ); 
                % Used to take away constant offset here (100)
            end
            ...
            if counter > 2*meta(1).stacksize
                ...
                % readjust the stack.
                if size(image,3) > initial_stackSize
                    image  = image(:,:,(size(image,3)-initial_stackSize+1):end);
                end
                if size(image2,3) > initial_stackSize
                    image2 = image2(:,:,(size(image2,3)-initial_stackSize+1):end);
                end
                ...
                name1 = fullfile(datafolder, ['Time_',sprintf('%06d',TimeCount+t_off),'_Angle_',num2str(angle+0*180),'_c',num2str(col),'_ls_',num2str(ls),'.ome.tif']) ;
                name2 = fullfile(datafolder, ['Time_',sprintf('%06d',TimeCount+t_off),'_Angle_',num2str(angle+1*180),'_c',num2str(col),'_ls_',num2str(ls),'.ome.tif']) ;
...
if counter ~=1 && ismember(TimeCount, timestamps_to_save)
    ...
    name1 = fullfile(datafolder,['Time_',sprintf('%06d',TimeCount),'_Angle_',num2str((p-1)*45+0*180),'_c',num2str(col),'_ls_',num2str(ls),'.ome.tif']);
    name2 = fullfile(datafolder,['Time_',sprintf('%06d',TimeCount),'_Angle_',num2str((p-1)*45+1*180),'_c',num2str(col),'_ls_',num2str(ls),'.ome.tif']);
