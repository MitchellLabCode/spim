...
def process_directory(input_dir, output_dir, dir_name, plane=-1):
    ...
    for filename in os.listdir(input_dir):
        pattern = re.compile(r"channel_(\d+)_tp_(\d+)_angle_(\d+)\.ome\.tif")

        match = pattern.match(filename)
        if match:
            ch = match.group(1)
            angle = match.group(3)
            if ch == '0' and angle == '47':
...
if __name__ == "__main__":
    ...
    parent_dir = "/mnt/data/to_deconvolve/wenjie/wenjie/2024-12-05_194949/unpacked_flipped_rotated_TIFFs_new"
